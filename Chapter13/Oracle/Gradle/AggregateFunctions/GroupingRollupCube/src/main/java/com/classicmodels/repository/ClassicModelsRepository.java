...
public class ClassicModelsRepository {
    ...
    public void groupingIdExample() {

        ctx.select(SALE.EMPLOYEE_NUMBER, SALE.FISCAL_YEAR,
                groupingId(SALE.EMPLOYEE_NUMBER, SALE.FISCAL_YEAR), sum(SALE.SALE_))
                .from(SALE)
                .groupBy(groupingSets(SALE.EMPLOYEE_NUMBER, SALE.FISCAL_YEAR))
                .fetch();
        ...
        ctx.select(case_().when(grouping(OFFICE.TERRITORY).eq(1), "{generated}")
                ...
                case_().when(grouping(OFFICE.CITY).eq(1), "{generated}")
                        .else_(OFFICE.CITY).as("city"),
                groupingId(OFFICE.TERRITORY, OFFICE.STATE, OFFICE.COUNTRY, OFFICE.CITY).as("grouping_id"),
                sum(OFFICE.INTERNAL_BUDGET))
                .from(OFFICE)
                .where(OFFICE.COUNTRY.eq("USA"))
                .groupBy(rollup(OFFICE.TERRITORY, OFFICE.STATE, OFFICE.COUNTRY, OFFICE.CITY))
        ...
        ctx.select(case_().when(grouping(OFFICE.TERRITORY).eq(1), "{generated}")
                ...
                .where(OFFICE.COUNTRY.eq("USA"))
                .groupBy(rollup(OFFICE.TERRITORY, OFFICE.STATE, OFFICE.COUNTRY, OFFICE.CITY))
                .having(groupingId(OFFICE.TERRITORY, OFFICE.STATE, OFFICE.COUNTRY, OFFICE.CITY).eq(3)) // group by territory and state
                .fetch();
    }
}
