...
public class ClassicModelsRepository {
    ...
    public void percentileRankEmployeesBySalary() {
        ...
        ctx.select(EMPLOYEE.FIRST_NAME, EMPLOYEE.LAST_NAME, EMPLOYEE.SALARY,
                round(cast(percentRank().over().orderBy(EMPLOYEE.SALARY), SQLDataType.NUMERIC), 2).as("percentile_rank"))
                .from(EMPLOYEE)
                .fetch();
    ...
    public void percentileRankEmployeesBySalaryAndOffice() {
        ...
        ctx.select(EMPLOYEE.FIRST_NAME, EMPLOYEE.LAST_NAME, EMPLOYEE.SALARY,
                ...
                round(cast(percentRank().over().partitionBy(OFFICE.OFFICE_CODE)
                        .orderBy(EMPLOYEE.SALARY).mul(100), SQLDataType.NUMERIC), 2).concat("%").as("percentile_rank"))
                .from(EMPLOYEE)
                .innerJoin(OFFICE)
