...
public class ClassicModelsRepository {
    ...
    public void view2() {
        ...
        ctx.dropViewIfExists("employee_with_sale_trend_change_year").execute();
        ctx.dropViewIfExists("employee_with_sale_trend_change_detection").execute();

        // compute the next year with trend changed        
        ctx.createView("employee_with_sale_trend_change_detection",
                "employee_number", "year", "trend", "has_changed")
                .as(select(SALE.EMPLOYEE_NUMBER, SALE.FISCAL_YEAR,
                        SALE.TREND, field(coalesce(lag(SALE.TREND)
                                .over().partitionBy(SALE.EMPLOYEE_NUMBER)
                                .orderBy(SALE.FISCAL_YEAR), "").ne(SALE.TREND)))
                        .from(SALE)
                        .orderBy(SALE.EMPLOYEE_NUMBER, SALE.FISCAL_YEAR))
                .execute();

        System.out.println(
                ctx.select().from(name("employee_with_sale_trend_change_detection")).fetch()
        );

        // use the previous view to compute the next year with trend change        
        ctx.createView("employee_with_sale_trend_change_year",
                "employee_number", "year", "trend", "change_year")
                .as(select(field(name("employee_number")), field(name("year")), field(name("trend")),
                        coalesce(min(field(name("year"))).filterWhere(field(name("has_changed"), Boolean.class))
                                .over().partitionBy(field(name("employee_number"))).orderBy(field(name("year")))
                                .rowsBetweenCurrentRow().andUnboundedFollowing().excludeCurrentRow(),
                                9999))
                        .from("employee_with_sale_trend_change_detection"))
                .execute();

        System.out.println(
                ctx.select().from(name("employee_with_sale_trend_change_year")).fetch()
        );

        ctx.select(field(name("employee_number")),
                min(field(name("year"))).as("from"), max(field(name("year"))).as("to"),
                min(field(name("trend"))).as("trend"),
                field(min(field(name("trend"))).eq(min(field(name("trend"))))).as("is_valid"))
                .from(name("employee_with_sale_trend_change_year"))
                .groupBy(field(name("employee_number")), field(name("change_year")))
                .orderBy(field(name("employee_number")), field(name("from")))
                .fetch();
    }
    ...
    public void view3() {
        ...
        ctx.createView("absent_values").as(
