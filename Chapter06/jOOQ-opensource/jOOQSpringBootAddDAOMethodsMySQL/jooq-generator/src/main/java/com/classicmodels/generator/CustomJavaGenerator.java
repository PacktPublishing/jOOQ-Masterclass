...
public class CustomJavaGenerator extends JavaGenerator {
    ...
    protected void generateDaoClassFooter(TableDefinition table, JavaWriter out) {
        ...
        out.println("public %s<%s> findLimitedTo(%s value) {", out.ref(List.class), pType, Integer.class);
        ...
        if (table.getName().equals("order")) {
            out.println();
            out.javadoc("Fetch orders having status <code>statuVal</code> and order date after <code>orderDateVal</code>");
