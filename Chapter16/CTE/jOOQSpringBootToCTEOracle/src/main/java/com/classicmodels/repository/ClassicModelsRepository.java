...
public class ClassicModelsRepository {
    ...
    public void derivedTableToTemporaryTableToCTE() {
        ...
        ctx.createTable("t3").as(
                ctx.select(EMPLOYEE.FIRST_NAME.as("first_name"), 
                        EMPLOYEE.LAST_NAME.as("last_name"), sum(SALE.SALE_).as("employee_sale"))
                        .from(EMPLOYEE)
                        .innerJoin(SALE)
                        .on(EMPLOYEE.EMPLOYEE_NUMBER.eq(SALE.EMPLOYEE_NUMBER))
    ...
    public void temporaryTableToCTE() {
        ...
        ctx.createTable("max_employee_sale").as(
                select(SALE.EMPLOYEE_NUMBER.as("employee_number"), max(SALE.SALE_).as("max_sale"))
                        .from(SALE)
