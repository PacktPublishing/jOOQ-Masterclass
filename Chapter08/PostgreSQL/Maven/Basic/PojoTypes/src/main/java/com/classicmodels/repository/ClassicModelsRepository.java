...
public class ClassicModelsRepository {
    ...
    public void fetchSimplePojoExamples() {
        ...
        List<FlatProductline> result5 = ctx.select(PRODUCTLINE.PRODUCT_LINE, PRODUCTLINE.CODE,
    ...
    public void fetchJavaRecordPojoExamples() {
        ...
        List<RecordDepartment> result3 = ctx.select(
                DEPARTMENT.NAME, DEPARTMENT.CODE, DEPARTMENT.TOPIC)
                .from(DEPARTMENT)
                .fetch(Records.mapping(RecordDepartment::new));
        System.out.println("Example 4.3\n" + result3);

        // UDT
        List<RecordManager> result4 = ctx.select(MANAGER.MANAGER_NAME, MANAGER.MANAGER_EVALUATION)
                .from(MANAGER)
                .fetchInto(RecordManager.class);
        System.out.println("Example 4.3\n" + result4);

        // embeddable type
        List<RecordOffice> result5 = ctx.select(OFFICE.OFFICE_CODE, OFFICE.OFFICE_FULL_ADDRESS)
                .from(OFFICE)
