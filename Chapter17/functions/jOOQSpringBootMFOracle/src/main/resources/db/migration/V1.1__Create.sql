...
(
 price number,  
 msrp number, 
 member function increase(p number) return product_obj, 
 map member function diff return number,
 member procedure display
); 
/ 

CREATE OR REPLACE TYPE BODY product_obj AS 
   MEMBER FUNCTION increase(p number) return product_obj IS 
   BEGIN 
      return product_obj(self.price + p, self.msrp + p); 
   END increase;     
   MAP MEMBER FUNCTION diff return number IS 
   BEGIN 
      return (sqrt(msrp*msrp - msrp*price)); 
   END diff; 
   MEMBER PROCEDURE display IS 
   BEGIN  
      dbms_output.put_line('Price: '|| price); 
      dbms_output.put_line('MSRP: '|| msrp); 
   END display;  
END; 
/ 
